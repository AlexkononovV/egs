...
async def read_books(db=Depends(db), title: Optional[str] = None, author:Optional[str]=None,category:Optional[str]=None):
    books = await crud.get_books(db, title=title, author=author, category=category)
    if books is None:
...
def read_book( bookId: int,db=Depends(db)):
    book = crud.get_book_id(db, id=bookId)
    if book is None:
...
def post_book(book: schemas.Book,db=Depends(db)):
    #book = crud.save_book(db, book)
    return crud.save_book(db, book)
...
@app.get('/v1/book/{bookId}/availability', response_model=List[schemas.Book])
def read_book(bookId: int, db=Depends(db)):
...
@app.post('/v1/author')
async def post_author(author: schemas.Author,db=Depends(db)):
...
def post_category(cat: schemas.Category,db=Depends(db)):

    return crud.save_category(db, cat)

@app.get('/v1/category', response_model=List[schemas.Category])
def get_category(id: Optional[int] =None, name: Optional[str]=None,db=Depends(db)):
    return crud.get_category(db, name, id)

@app.delete('/v1/category/{id}' )
def delete_book(id:int,db=Depends(db)):
    msg = crud.delete_category(db, id)
    return msg